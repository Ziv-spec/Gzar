...
internal Expr *parse_assignment(Translation_Unit* tu) {
    ...
    if (match(tu, TK_ASSIGN)) {
        return init_assignment(&tu->m, expr, parse_logical(tu));
    }
    return expr;
}
...
internal Expr *init_binary(M_Pool *mem, Expr *left, Token operation, Expr *right) {
    Expr *result_expr = (Expr *)pool_alloc(mem, sizeof(Expr));
...
internal Expr *init_assignment(M_Pool *mem, Expr *left_var, Expr *rvalue) {
    Expr *result_expr = (Expr *)pool_alloc(mem, sizeof(Expr));
...
internal Statement *parse_return_stmt(Translation_Unit* tu) {
    ...
    return init_return_stmt(&tu->m, expr);
}

internal Statement *parse_expr_stmt(Translation_Unit* tu) {
    Expr *expr = parse_expression(tu);
    consume(tu, TK_SEMI_COLON, "Expected ';' after a statement");
    return init_expr_stmt(&tu->m,expr);
}

internal Statement *init_while_stmt(M_Pool *m, Expr *condition, Statement *block) {
    Statement *stmt = (Statement *)pool_alloc(m, sizeof(Statement));
...
internal Statement *init_if_stmt(M_Pool *m, Expr *condition, Token position, Statement *true_block,  Statement *false_block) {
    Statement *stmt = (Statement *)pool_alloc(m, sizeof(Statement));
    stmt->kind = STMT_IF;
...
internal Statement *init_expr_stmt(M_Pool *m,Expr *expr) {
    Statement *stmt = (Statement *)pool_alloc(m, sizeof(Statement));
    stmt->kind = STMT_EXPR;
    stmt->expr = expr;
    return stmt;
...
internal Statement *init_func_decl_stmt(M_Pool *m,Token name, Type *ty, Statement *sc) {
    Statement *stmt = (Statement *)pool_alloc(m, sizeof(Statement));
...
internal Statement *init_var_decl_stmt(M_Pool *m,Symbol *symb) {
    Statement *stmt = (Statement *)pool_alloc(m, sizeof(Statement));
...
internal Statement *init_scope(M_Pool *m) {
    Statement *stmt = (Statement *)pool_alloc(m, sizeof(Statement));
    stmt->kind = STMT_SCOPE;
    stmt->block.statements = vec_init();
...
internal Statement *init_return_stmt(M_Pool *m,Expr *expr) {
    Statement *stmt = (Statement *)pool_alloc(m, sizeof(Statement));
...
internal Symbol *init_symbol(M_Pool *m,Token name, Type *type, Expr *initializer) {
    Symbol *symbol = (Symbol *)pool_alloc(m, sizeof(Symbol));
    symbol->name = name;
    symbol->type = type;
...
internal Type *init_type(M_Pool *m,Type_Kind kind, Type *subtype, Vector *symbols) {
    Type *ty = (Type *)pool_alloc(m, sizeof(Type));
    ty->kind = kind;
    ty->subtype = subtype;
    ty->symbols = symbols;
    return ty;
