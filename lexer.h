...
enum Token_Kind {
    ...
    TK_LESS,    // <
    TK_ASSIGN,  // = 
    // TODO(ziv): add this to the parser !!!!!!!!!!!!!!!!!!!!!!!!!
    TK_GREATER_EQUAL, // >=
    TK_LESS_EQUAL,    // <=
    TK_BANG_EQUAL,    // != 
    TK_EQUAL_EQUAL,   // ==
    ...
    TK_RBRACE, // {
    TK_LBRACE, // }
    // TODO(ziv): Check whether parsing an a identifier is working
...
internal bool get_next_token(Lexer *lexer) { 
    ...
    switch (c) {
        ...
        case '/': { 
            if (lexer->code[loc.index+1] == '/') { // single line comment
                while (lexer->code[loc.index++] != '\n'); 
            }
            else {
                token.kind = TK_SLASH; 
            }
